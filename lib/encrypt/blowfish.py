...
class Blowfish:
    ...
    def check_key(self, key):
        return (len(key) == self.keysize)

    def F(self, f1):
        s0 = self.s[0][(f1&0xFF000000)>>24]
        s1 = self.s[1][(f1&0X00FF0000)>>16]
        s2 = self.s[2][(f1&0x0000FF00)>>8]
        s3 = self.s[3][(f1&0x000000FF)]
        if s0 > s1:
            if s0 > 2147483647:
                s0 -= 4294967296
        else:
    ...
    def round(self, f1, f2, f3):
        return (f1, (f2 ^ (self.p[f3])) ^ self.F(f1))

    def crypt_block(self, l, r, d):
        if not d:
            l ^= self.p[0]
            for i in range(1, 17, 1):
                (r, l) = self.round(l, r, i)
            r ^= self.p[17]
        else:
            l ^= self.p[17]
            for i in range(1, 17, 1):
                (r, l) = self.round(l, r, 17-i)
            r ^= self.p[0]
        return (r, l)
    ...
    def encrypt(self, block):
        ...
        r = parts[7] | (parts[6] << 8) | (parts[5] << 16) | (parts[4] << 24)
        (l, r) = self.crypt_block(l, r, 0)
        p = (
            l >> 24, l >> 16, l >> 8, l,
            r >> 24, r >> 16, r >> 8, r,
...
if __name__ == '__main__':
   ...
   g = bfkey.decrypt(f)
   assert plaintext == g, "Could not encrypt/decrypt properly"
